 ...
 */
 ...
 (function($)
 {
 	...
 	function setToCurrentSlideHeight(t,c){
 		t.stop().animate({'height':c.height()+'px'});
 	}
	/*
	  ...
	  function ltrAnimation(t, l, opts)
	  {
	  	...
	  	function rtlAnimation(t, l, opts)
	  	{
	  		...
	  		function nextaxeBlinds(t, l, opts)
	  		{
	  			...
	  			$(l).stop(true,true).animate({ 'width' : opts['inactw'] + '%' },
	  				opts['o'], opts['ezing'], function()
	  				{
 ...
 (function($)
 {
 	$.fn.axeCarousel = function(options)
 	{
 		...
 		$('.axecarouselnext',t).click(function(e){
 			e.preventDefault();
 			if(opts['dir'] == 'rev' && opts['ispaused'] == 0){ return;}
 			opts['dir'] = 'rev';
 			clearTimeout(opts['tvar']);
 		...
 		$('.axecarouselback',t).click(function(e){
 			e.preventDefault();
 			if(opts['dir'] == 'norm' && opts['ispaused'] == 0){ return;}
 			opts['dir'] = 'norm';
 			clearTimeout(opts['tvar']);
 			showCarouselLoader(t, l, opts);
 			$('.' + opts['childcontainer'], t).stop(true,true);
 	...
 	function setupaxeCarousel(t, l, opts, resized)
 	{
 		...
 		if (len)
 		{
 			switch (opts['mode'])
 			{
 				...
 				ch = t.width();
 				cdh = $(l[opts['cslide']]).outerWidth(true) + 5;
 				/*console.log(ch + ' ' + cdh);*/
 				df = Math.ceil(ch / cdh);
 				$('.' + opts['childcontainer'], t).css(
 					{ 'width' : (cdh * len) + 'px' ,
 					/*'height' : t.height() + 'px' ,*/
 					'position' : 'relative' });
 				$('.ohidden',t).css({'height':$(l[opts['cslide']]).outerHeight()+'px'});
 				break;
 	...
 	function animateToTop(t,l,opts,animto){
 		$('.' + opts['childcontainer'], t).stop().animate({'top': animto + 'px' }, 600, opts['ezing'],function(){
 			$(this).css({'top':'0px'}).stop(true,true);
 			setCSlide(t,l,opts);
 			moveTopItemsToBottom(t,l,opts);
 		});
 	}
 	function animateToLeft(t,l,opts,animto){
 		$('.' + opts['childcontainer'], t).stop().animate({'left': animto + 'px' }, 600, opts['ezing'],function(){
 			$(this).css({'left':'0px'}).stop(true,true);
 			setCSlide(t,l,opts);
 			moveTopItemsToBottom(t,l,opts);
 		});
 	}
 	...
 	function axeRevVerticalCarousel(t, l, opts)
 	{
 		if (parseInt(opts['tslides']) > 1)
 		{
 			if(opts['lock'] == 0){
 				...
 				hideCarouselLoader(t, l, opts);
 				ct.prepend($(l[opts['mnslide']])).css({ 'top' : '-' + ti + 'px' }).stop(true,true).animate(
 			...
 			if(opts['lock'] == 0){
 				...
 				$('.' + opts['childcontainer'], t).stop(true,true).animate(
 					{ 'left' : ti + 'px' }, td, opts['ezing'], function()
 					{
 						...
 						/* Adjust Left */
 						$(this).css({ 'left' : '0px' });
 						$(this).parent('.ohidden').animate({'height':$(l[opts['cslide']]).outerHeight()+'px'});
